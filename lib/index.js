...
const onRemove = (val, props, state, setState) => {
	...
	if (onChange) {
		onChange({[name]: uniq(active.map(x => x.value), (a, b) => !deepEqual(a, b))}, val);
	}
...
const getText = (props, state, setState) => {
	...
	if (active.length !== 0 && multiple && showValue) {
		return active.map(x => {
			return (
				<div class='Select-multiple'>
					<div class='Select-remove' onClick={() => onRemove(x, props, state, setState)}>
						{symbol}
					</div>
					{x.label}
				</div>
...
const onClick = (option, props, state, setState) => {
	...
	if (arr.some(x => deepEqual(x, option.value))) {
		...
		if (onChange) {
			onChange({[name]: uniq(active.map(x => x.value), (a, b) => !deepEqual(a, b))}, option);
		}
	...
	if (onChange) {
		onChange({[name]: uniq(ret.map(x => x.value), (a, b) => !deepEqual(a, b))}, option);
	}
};
...
const openSelect = ({open, toggle}, setState) => () => setState({open: toggle === true ? !open : true});
...
const render = ({props, state}, setState) => {
	...
	return (
		<div class={['Select', props.class]} id={id}>
			<div class='Select-input' onClick={openSelect({open, toggle}, setState)}>
